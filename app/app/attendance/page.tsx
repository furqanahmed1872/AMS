...
export default function AttendancePage() {
  ...
  const markAll = (status: "P" | "A" | "L") =>
    setAttendance(
      Object.fromEntries(MONTHLY_ATTENDANCE.map((s) => [s.studentId, status])),
    );
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              {
                label: "Total",
                count: MONTHLY_ATTENDANCE.length,
                color: "text-white/70",
                bg: "bg-white/5",
              },
                  ...
                  <tr key={student.studentId} className="table-row">
                    ...
                    {days.map((d) => {
                      ...
                      return (
                        <td
                          key={d}
                          className={`px-2 py-3 text-center font-bold ${s === "P" ? "text-emerald-400" : s === "A" ? "text-rose-400" : s === "L" ? "text-amber-400" : "text-white/20"}`}
                        >
                          {s || "—"}
                        </td>
                      );
