...
export default function MarksEntryPage({ params }: { params: { id: string } }) {
  ...
  const students = STUDENTS.filter(s => s.classId === test.classId && s.status === "active");

  const [marks, setMarks] = useState<Record<string, string>>({});
  const [absent, setAbsent] = useState<Record<string, boolean>>({});

  const entered = Object.values(marks).filter(v => v !== "").length + Object.values(absent).filter(Boolean).length;
  ...
  return (
    <div className="space-y-5 animate-fade-in max-w-2xl">
      <PageHeader
        ...
        back={<Link href="/app/tests"><button className="p-2 hover:bg-white/8 rounded-xl transition-colors"><ArrowLeft size={18} /></button></Link>}
        actions={
          <div className="flex items-center gap-3">
            <span className="text-xs text-white/40 font-medium">{entered}/{students.length} entered</span>
            <Button icon={<Save size={14} />} size="sm">Save</Button>
          </div>
        }
      />
      ...
      <Card className="overflow-hidden">
        <div className="divide-y divide-white/5">
          {students.map((student, i) => {
            ...
            const pct = mark && !isAbsent ? Math.round((parseFloat(mark) / test.totalMarks) * 100) : null;
            return (
              <div key={student.id} className="flex items-center gap-4 px-4 py-3">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs text-white/30">/{test.totalMarks}</span>
                  </div>
                  <div className="w-14 text-right">
                    {isAbsent ? <span className="text-xs font-medium text-rose-400">Absent</span>
                      : pct !== null ? <span className={`text-xs font-bold ${pct >= 33 ? "text-emerald-400" : "text-rose-400"}`}>{pct}%</span>
                      : <span className="text-xs text-white/20">—</span>}
                  </div>
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    <input type="checkbox" checked={isAbsent} onChange={e => setAbsent(prev => ({ ...prev, [student.id]: e.target.checked }))}
      ...
      <Button icon={<Save size={15} />} className="w-full">Save All Marks</Button>
    </div>
  );
}
